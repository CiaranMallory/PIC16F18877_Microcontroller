...
#define PB2 RE0
#define PB3 RB0
// Macro for LED connected to PORTC position 0
#define LED RC0
...
void main(void) 
{
    ...
    TRISC = 0x00;
    ANSELE = 0x00;
    TRISE = 0xFF; // TRISE = Input
    
    LATC = 0x00; // LATC = Output
            
    while(1) { // Loop forever
        if(SW1 == ON || SW2 == ON || PB2 == ON || PB3 == ON){
            // If any of the given switches and pushbuttons are ON
            // Turn LED on
            LED = ON;
        }else{
